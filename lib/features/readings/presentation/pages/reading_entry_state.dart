...
class _FirebaseSnap {
  final bool exists;
  final dynamic value;
  _FirebaseSnap(this.exists, this.value);
}
...
class _RefStub {
  ...
  Future<_FirebaseSnap> get() async {
    final val = _db[path];
    return _FirebaseSnap(val != null, val);
  }
  ...
  Future<void> update(Map<String, dynamic> updates) async {
    if (_db[path] is Map) {
      (_db[path] as Map).addAll(updates);
    } else {
      _db[path] = updates;
    }
  }
...
_RefStub _ref(String path) => _RefStub(path);

double _calcRankScore(String severity, int rating, String timeRange, String dueDateStr) => 50.0;
...
class _ReadingEntryScreenState extends State<ReadingEntryScreen> {
  ...
  late final String _readingId = const Uuid().v4();

  Future<String> _uploadFile(
    File file, {
    String category = 'violation_image',
    String? paramId,
    String? imageId,
  }) async {
    final bytes = await file.readAsBytes();
    final url = await ReadingRepository().uploadReadingImage(
      bytes: bytes,
      category: category,
      assetId: widget.tank.id,
  ...
  Future<void> _captureParamImage(String paramId) async {
    XFile? img;
    try {
      img = await _picker.pickImage(source: ImageSource.camera, imageQuality: 90);
    } catch (e) {
      if (mounted) _snack('Camera error: $e', _kDanger);
      return;
    }
    ...
    try {
      final url = await _uploadFile(annotated, category: 'auto_capture_image', imageId: paramId);
      if (mounted) {
        setState(() {
  ...
  Future<void> _captureViolationPhoto(
    ...
    try {
      final url = await _uploadFile(annotated, category: 'violation_image', imageId: '${paramId}_$constraintId');
      if (mounted) {
        setState(() {
  ...
  Future<void> _captureManualPhoto(int index) async {
    ...
    try {
      final url = await _uploadFile(annotated, category: 'manual_capture_image', imageId: 'manual_$index');
      if (mounted) {
        setState(() {
  ...
  Future<void> _writeLiveAlertWithPhoto({
    ...
    final rankScore = _calcRankScore(
      v.severity,
      rating,
      timeRange,
      dueDateStr,
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      for (final p in _allActiveProps()) {
        ...
        final minVal = p['min'] != null ? (p['min'] as num).toDouble() : null;
        final maxVal = p['max'] != null ? (p['max'] as num).toDouble() : null;
        final val = _currentValue(id, type, p);
        final imgUrl = _paramPhotoUrl[id];

        final hasViolations = (_violations[id] ?? []).isNotEmpty;
        final violationMsg = hasViolations ? _violations[id]!.map((v) => v.message).join('; ') : null;

        valuesList.add(ReadingValueModel(
          id: const Uuid().v4(),
          paramId: id,
          paramLabel: label,
          paramType: type,
          val: val.toString(),
          numericVal: double.tryParse(val.toString()),
          unit: unit,
          minVal: minVal,
          maxVal: maxVal,
          isViolation: hasViolations,
          violationMessage: violationMsg,
          imageUrl: imgUrl,
        ));

        if (imgUrl != null && imgUrl.isNotEmpty) {
          imagesList.add(ReadingImageModel(
            id: const Uuid().v4(),
            paramId: id,
            category: 'auto_capture_image',
            imageUrl: imgUrl,
          ));
        }
      }

      _violationPhotoUrls.forEach((paramId, map) {
        map.forEach((constraintId, imgUrl) {
          if (imgUrl != null && imgUrl.isNotEmpty) {
            imagesList.add(ReadingImageModel(
              id: const Uuid().v4(),
              paramId: paramId,
              category: 'violation_image',
              imageUrl: imgUrl,
            ));
          }
        });
      ...
      for (final entry in _manualCaptures) {
        if (entry.uploadedUrl != null && entry.uploadedUrl!.isNotEmpty) {
          imagesList.add(ReadingImageModel(
            id: const Uuid().v4(),
            paramId: entry.selectedParamId,
            category: 'manual_capture_image',
            imageUrl: entry.uploadedUrl!,
          ));
        }
      }
